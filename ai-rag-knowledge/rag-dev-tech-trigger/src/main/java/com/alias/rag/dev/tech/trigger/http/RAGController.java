...
public class RAGController implements IRAGService {
  ...
  @Operation(
      ...
      description =
          "克隆指定的 Git 仓库，遍历仓库中的所有文件，解析文件内容并导入到知识库中。知识库标签会自动从仓库 URL 中提取（通常是仓库名称）。该操作会删除本地临时克隆的仓库。")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
            description = "Git 仓库分析并导入成功",
            content = @Content(schema = @Schema(implementation = Response.class))),
  ...
  public Response<String> analyzeGitRepository(
      @Parameter(
              description = "Git 仓库 URL，例如：https://github.com/user/repo.git",
              required = true,
              example = "https://github.com/user/repo.git")
          @RequestParam("repoUrl")
          String repoUrl,
          ...
          public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) {
            ...
            try {
              ...
              documentSplitterList.forEach(
                  doc -> doc.getMetadata().put("knowledge", repoProjectName));
