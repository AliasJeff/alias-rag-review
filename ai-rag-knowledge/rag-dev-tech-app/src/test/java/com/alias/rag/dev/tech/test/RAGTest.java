...
public class RAGTest {
  ...
  public void chat() {
    ...
    SearchRequest request =
        SearchRequest.builder()
            .query(message)
            .topK(5)
            .filterExpression("knowledge == 'ai-rag-knowledge'")
            .build();

    List<Document> documents = pgVectorStore.similaritySearch(request);
    String documentsCollectors =
        documents.stream().map(Document::getFormattedContent).collect(Collectors.joining());
  ...
  public void test_testQueryDocuments() {
    ...
    SearchRequest request =
        SearchRequest.builder().query("search all").topK(5).filterExpression(filter).build();
